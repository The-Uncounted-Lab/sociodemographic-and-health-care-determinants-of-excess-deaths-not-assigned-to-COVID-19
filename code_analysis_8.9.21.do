********************************************************************************************************
** Analytic Code for:
** Covid-19 and Excess Mortality in the United States: An Analysis of Counties (PLOS MEDICINE)
** Association of Health Care Factors with Excess Deaths Not Assigned to COVID-19 (JAMA NETWORK OPEN)
...
/* Source: https://data.cdc.gov/NCHS/AH-County-of-Residence-Provisional-COVID-19-Deaths/75vb-d79q */
...
///////////////////////////////////////////
// PART 6: PRIMARY ANALYSIS 	         //
// STRATIFICATION BY HEALTH CARE FACTORS //
///////////////////////////////////////////
...
// 12. CDC Wonder Data on % of All-Cause Deaths Occuring at Home
/* Underlying Cause of Death Data, Select Years 2013-2018, Select Home as Place of Death, Sort by Counties */
import delimited home_1318.txt, clear
keep countycode deaths
drop if deaths == .
rename countycode fipscountycode
rename deaths deaths_home
save home_1318.dta, replace

// 13. CDC Wonder Data on % of All-Cause Deaths Occuring in Hospitals
/* Underlying Cause of Death Data, Select Years 2013-2018, Select Inpatient as Place of Death, Sort by Counties */
import delimited inpatient_1318.txt, clear
keep countycode deaths
drop if deaths == .
rename countycode fipscountycode
rename deaths deaths_inpatient
save inpatient_1318.dta, replace

// 14. CDC Wonder Data on % of All-Cause Deaths Occuring in Nursing Homes
/* Underlying Cause of Death Data, Select Years 2013-2018, Select Nursing Home as Place of Death, Sort by Counties */
import delimited nursinghome_1318.txt, clear
keep countycode deaths
drop if deaths == .
rename countycode fipscountycode
rename deaths deaths_nursinghome
save nursinghome_1318.dta, replace

// 15. 2020 RWJ Foundation County Health Rankings
/* Source: https://www.countyhealthrankings.org/explore-health-rankings/rankings-data-documentation */
import delimited "5_RWJF_county_health_rankings.csv", clear
rename fipscode fipscountycode 
rename v085_rawvalue percent_uninsured
rename v004_rawvalue primary_care
rename v005_rawvalue prevent_hospitals
keep fipscountycode percent_uninsured primary_care prevent_hospitals
save uninsured.dta, replace

// 16. University of Minnesota COVID-19 Hospitalization Tracking Project
/* Source: https://carlsonschool.umn.edu/mili-misrc-covid19-tracking-project */
/* Source: https://drive.google.com/drive/folders/12XBifvy1PlP-R7RAa4gqcK7jFHLPxbA5 */
/* Used File from December 2020 */
import delimited "hospital_capacity.csv", clear
rename fipscode fipscountycode
keep fipscountycode totalhospitalbeds
save hospitals.dta, replace

// 17. Death Investigation Systems: Presence of a State Medical Examiner 
/* Source: https://www.cdc.gov/phlp/publications/coroner/death.html */
import delimited examiners.csv, clear varname(1)
encode examiner, gen(state_examiner)
keep state state_examiner
drop if state == ""
save examiners_state.dta, replace

// 18. County MDI System: Presence of a Coroner or Medical Examiner
/* Source: Generated by Stokes et al. using publicly available county data */
import excel MDI_database_8.4.21.xlsx, clear
drop if A == "All United States Counties Death Investigation Systems" | A == "STCOU" | A == ""
rename A fipscountycode
generate county_MDI = "Coroner Office" if D == "Appointed Coroner office" | D == "Appointed coroner office" | D == "Coroner" | D == "Coroner office" | D == "Coroner officer" | D == "Justice of the Peace" | D == "Sheriff-Coroner" | D== "Sheriff-coroner"
replace county_MDI = "Medical Examiner" if D == "Elected Medical Examiner" | D == "Medical Examiner" | D == "Medical examiner" | D == "State medical examiner"
keep fipscountycode county_MDI
destring fipscountycode, replace
save county_MDI.dta, replace
...
use data_clean.dta, clear
drop poor_or_fair_health smoking obesity some_college diabetes household_income home_ownership percent_65_over percent_black percent_hispanic percent_white percent_rural region

// Merge in Health Care Factors
merge 1:1 fipscountycode using inpatient_1318.dta
keep if _merge == 3
drop _merge
merge 1:1 fipscountycode using nursinghome_1318.dta
keep if _merge == 3
drop _merge
merge 1:1 fipscountycode using home_1318.dta
keep if _merge == 3
drop _merge
merge 1:1 fipscountycode using uninsured.dta
keep if _merge == 3
drop _merge
merge 1:1 fipscountycode using hospitals.dta
keep if _merge == 1 | _merge == 3
drop _merge
merge m:1 state using examiners_state.dta
keep if _merge == 3
drop _merge
merge 1:1 fipscountycode using county_MDI.dta
keep if _merge == 3
drop _merge

// Format % of Deaths at Home, in Nursing Homes, and at Hospitals
replace deaths_inpatient = deaths_inpatient / deaths_13_18
replace deaths_nursinghome = deaths_nursinghome / deaths_13_18
replace deaths_home = deaths_home / deaths_13_18

// Format Number of Hospital Beds per Capita 
replace totalhospitalbeds = totalhospitalbeds / pop_2020

// Create Coroner Variable for Coroners in States without Medical Examiners
gen coroner = 0
replace coroner = 1 if county_MDI == "Medical Examiner" & state_examiner == 2
save data_jno.dta, replace
...
// Divide Health Care Factors into Population Weighted Quartiles
use data_jno.dta, clear
foreach lname in deaths_home deaths_inpatient deaths_nursinghome percent_uninsured primary_care totalhospitalbeds prevent_hospitals {
xtile `lname'_up = `lname' [aweight=pop_2020], nq(4)
}
...
reg all_2020_death_rate all_hist_death_rate covid_2020_death_rate [weight = pop_2020] if coroner == 0
reg all_2020_death_rate all_hist_death_rate covid_2020_death_rate [weight = pop_2020] if coroner == 1
...
regress all_2020_death_rate all_hist_death_rate covid_2020_death_rate [weight=pop_2020] if coroner == 0
mat results = r(table)
summarize covid_2020_death_rate [weight = pop_2020] if coroner == 0
gen output = (results[1,2] - 1) * r(mean)
mean output
drop output
regress all_2020_death_rate all_hist_death_rate covid_2020_death_rate [weight=pop_2020] if coroner == 1
mat results = r(table)
summarize covid_2020_death_rate [weight = pop_2020] if coroner == 1
gen output = (results[1,2] - 1) * r(mean)
mean output
drop output
...
// Figure 1
use data_jno.dta, clear
foreach lname in deaths_home deaths_inpatient deaths_nursinghome totalhospitalbeds percent_uninsured primary_care prevent_hospitals {
xtile `lname'_up = `lname' [aweight=pop_2020], nq(4)
}
gen estimate = .
gen lower_bound = .
...
forvalue num2 = 0/1 {
regress all_2020_death_rate all_hist_death_rate covid_2020_death_rate [weight=pop_2020] if coroner == `num2'
mat results = r(table)
replace variable = "coroner" if indicator == `num'
replace level = "`num2'"  if indicator == `num'
replace estimate = results[1,2] if indicator == `num'
replace lower_bound = results[5,2] if indicator == `num'
replace upper_bound = results[6,2] if indicator == `num'
...
gen percent_underreport = percent1 + "% (" + percent2 +"%, " + percent3 + "%)"
drop percent1 percent2 percent3
replace variable = "% of All-Cause Deaths Occuring at Home" if variable == "deaths_home"
replace variable = "% of All-Cause Deaths Occuring at Hospitals" if variable == "deaths_inpatient"
replace variable = "% of All-Cause Deaths Occuring in Nursing Homes" if variable == "deaths_nursinghome"
replace variable = "Number of Hospital Beds per Capita" if variable == "totalhospitalbeds"
replace variable = "% Uninsured" if variable == "percent_uninsured"
replace variable = "Primary Care Physicians per Capita" if variable == "primary_care"
replace variable = "Preventable Hospital Stays per Medicare Enrollee" if variable == "prevent_hospitals"
replace level = "No" if level == "0" & variable == "coroner"
replace level = "Yes" if level == "1" & variable == "coroner"
replace variable = "State & County Medical Examiner" if variable == "coroner"
label var variable "Indicators"
label var level "Factors"
label var variable "Factors"
label var percent_underreport "Percent of Excess Deaths Not Assigned to Covid-19 (95% CI)"
metan estimate lower_bound upper_bound, by(variable)
metan estimate lower_bound upper_bound, by(variable) nooverall nosubgroup xline(1, lcolor(black)) xline(1.20, lcolor(navy) lpattern(shortdash)) xlabel(0, 1, 2) nulloff effect("Coefficients Relating Excess Deaths to Covid-19 Deaths") lcols(level) rcols(percent_underreport) graphregion(color(white)) xsize(14) ysize(10) boxsca(0.8) scale(1.2) force pointopt(msize(0.8)) ciopt(lwidth(medthin)) 
graph export jno_figure_1.jpg, replace width(3000)
graph export jno_figure_1.eps, replace
...
gen directcoeff = results[1,2]
replace directcoeff = directcoeff - 1
mean covid_2020_death_rate [weight = pop_2020] if coroner == `num2'
mat results = r(table)
replace direct_bar = results[1,1] if indicator == `num'
replace notassigned_bar = direct_bar * directcoeff if indicator == `num'
drop directcoeff
...
graph hbar direct_bar notassigned_bar, stack over(level, sort(indicator)) over(variable, sort(indicator)) ysize(8) xsize(13) graphregion(color(white)) scale(0.6) legend(label(1 "Observed Direct Covid-19 Death Rate") label(2 "Predicted Death Rate Not Assigned to Covid-19") col(1)) nofill b1title("Deaths per 1000 Person-Years")
graph export jno_figure_2.jpg, replace width(3000)
graph export jno_figure_2.eps, replace
